...
test(`Modify All Data Element`, async ({ page, materialPage, navigationMenu, auditTab, searchPage }) => {
    await test.step(`modify all`, async () => {
        ...
        await expect(materialPage.matDialog()).toContainText('Selected: 3 Data Elements');
        ...
        await expect(page.locator('cde-data-element-view')).toContainText('ON THIS PAGE');
    ...
    await test.step(`audit`, async () => {
        ...
        await expect(page.locator('cde-app-log')).toContainText('Running Batch Modify from search "/cde/search?');

        await auditTab.cdeAuditLog().click();
        await button(page.locator('cde-item-log'), 'Search').click();
        await expect(page.locator('cde-item-log')).toContainText('Education year count');
        await expect(page.locator('cde-item-log')).toContainText('Age value');
        await expect(page.locator('cde-item-log')).toContainText('Household member total count');
    });
});
