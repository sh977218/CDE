...
task('nodeVersion',function(cb){
    exec("node --version",(err, stdout) => {
        console.log('node version: ' + stdout);
        cb(err);
    });
});
task('npmCacheVerify',function(cb){
    exec("npm cache verify",(err, stdout) => {
        console.log('npm cache verify result: ' + stdout);
        cb(err);
    });
});

task('npmInstall',function(cb){
    exec("npm install",(err, stdout) => {
        console.log('npm install result: ' + stdout);
        cb(err);
    });
});

task('npmRebuildNodeSass', function(cb){
    exec("npm rebuild node-sass",(err, stdout) => {
        console.log('npm rebuid node sass result: ' + stdout);
        cb(err);
    });
});

task('thirdParty', function () {
    let streamArr = [];
...
task('prepare',series('nodeVersion','npmCacheVerify','npmInstall','npmRebuildNodeSass','thirdParty','createDist'));
...
task('default', series('copyNpmDeps', 'prepareVersion', 'copyUsemin', 'checkDbConnection'));
