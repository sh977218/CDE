...
export class CdeStatusReportComponent implements OnInit {
    gridOptionsReport = {
        columnDefs: [
            { field: 'cdeName', displayName: 'CDE Name' },
            { field: 'tinyId', displayName: 'NLM ID' },
        ],
    };
    message?: string;
                    ...
                    this.exportSvc.exportSearchResults(
                        ...
                        (records?: ExportRecord[]) => {
                            if (!records) {
                                this.state = 'message';
                                this.message =
                                    'Validation finished with Errors. This is possibly due to a network error. Please try again.';
                                return;
                            }
                            if (records.length === 0) {
                                this.state = 'message';
