...
export class FormUpdateCdeVersionModalComponent {
    ...
    doMerge(newQuestion: FormQuestion, currentQuestion: FormQuestion, newCde: DataElement, oldCde: DataElement) {
        ...
        newQuestion.question.unitsOfMeasure =
            newQuestion.question.unitsOfMeasure.length &&
            currentQuestion.question.unitsOfMeasure.filter(
        ...
        this.updateCdeVersion.bDefault =
            toString(currentQuestion.question.defaultAnswer) !== toString(newQuestion.question.defaultAnswer);
        this.updateCdeVersion.bUom = !isEqual(
            currentQuestion.question.unitsOfMeasure,
            newQuestion.question.unitsOfMeasure
        );
        ...
        switch (newQuestion.question.datatype) {
            case 'Value List':
                if (currentQuestion.question.datatype === 'Value List') {
                    this.updateCdeVersion.bValuelist = !isEqual(
                        currentQuestion.question.cde.permissibleValues,
                    ...
                    if (
                        currentQuestion.question.defaultAnswer &&
                        newQuestion.question.answers.filter(
                            a => a.permissibleValue === currentQuestion.question.defaultAnswer
                        ).length > 0
                    ) {
            ...
            case 'Number':
                if (
                    currentQuestion.question.datatype === 'Number' &&
                    currentQuestion.question.datatypeNumber &&
                    newQuestion.question.datatypeNumber
                ) {
                    this.updateCdeVersion.bNumberMin =
                        currentQuestion.question.datatypeNumber.minValue !==
                        newQuestion.question.datatypeNumber.minValue;
                    this.updateCdeVersion.bNumberMax =
                        currentQuestion.question.datatypeNumber.maxValue !==
