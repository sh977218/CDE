...
export class ScoreService {
    ...
    register(elt: CdeForm) {
        ...
        formScoreQuestions.forEach(formScoreQuestion => {
            formScoreQuestion.question.cde.derivationRules.forEach(
                derivationRule => {
    ...
    triggerCalculateScore(question: FormQuestion) {
        const scoreQuestions: FormQuestion[] | undefined =
            this.INPUT_SCORE_MAP.get(question.question.cde.tinyId);
        if (scoreQuestions) {
            scoreQuestions.forEach(scoreQuestion =>
                ScoreService.scoreSet(scoreQuestion, this.elt)
            );
        }
    }
        ...
        question.question.cde.derivationRules.forEach(async derRule => {
            if (derRule.ruleType === 'score') {
                if (derRule.formula === 'sumAll') {
                    const { error, sum } = ScoreService.sum(
                        derRule.inputs,
                        elt
                    );
                    return cb(error, sum);
                ...
                if (derRule.formula === 'mean') {
                    ...
                    return cb(
                        result.error,
                        result.sum !== undefined && !Number.isNaN(result.sum)
                            ? result.sum / derRule.inputs.length
                            : result.sum
                    );
                }
                if (derRule.formula === 'bmi') {
                    const aQuestion = findQuestionByTinyId(
                        derRule.inputs[0],
                        elt
                    );
                    if (!aQuestion) {
                    ...
                    const bQuestion = findQuestionByTinyId(
                        derRule.inputs[1],
                        elt
                    );
                    if (!bQuestion) {
                    ...
                    Promise.all([
                        ScoreService.ucumConverter(
                            aAnswer,
                            aQuestion.question.answerUom.code,
                            'kg'
                        ),
                        ScoreService.ucumConverter(
                            bAnswer,
                            bQuestion.question.answerUom.code,
                            'm'
                        ),
                    ]).then(values => {
    ...
    static scoreSet(question: FormQuestion, elt: CdeForm) {
        ScoreService.calculateScore(
            question,
            elt,
            (err: string | undefined, sum: number | void) => {
                if (err) {
    ...
    static sum(tinyIds: string[], elt: CdeForm): ErrorOrScore {
        ...
        tinyIds.forEach(cdeTinyId => {
            ...
            if (!q) {
                return (error =
                    'Cannot find ' + cdeTinyId + ' in form ' + elt.tinyId);
            }
