...
const toolTipMap = new Map([
    ...
    [
        'NIH-Endorsed',
        'NIH-endorsed CDEs have been reviewed and approved by the NIH CDE Governance Committee.',
    ],
    ['Name', 'The name of the CDE/Form'],
    [
        'Question Texts',
        'Standardized question text/field name for data collection',
    ],
    [
        'Permissible Values',
        'List of allowable answers, if the CDE data type is value list',
    ],
    ...
    [
        'Identifiers',
        'External identifiers if the CDE/Form is available from another source',
    ],
    [
        'Other Names',
        'alternative names for the CDE/Form (e.g., from other sources)',
    ],
    ['Updated', 'date updated in the Repository'],
    ['Code Names', 'code that represents the permissible value'],
    [
        'Unit of Measure',
        'e.g., cm, months, percent, breaths per minute, millimeter of mercury, ng/mL',
    ],
...
export class TableListComponent implements OnInit {
        ...
        this.rows = (this.elts as DataElementElastic[]).map(e => {
            ...
            if (tableSetup.naming) {
                row.push({
                    css: 'naming',
                    values: TableListComponent.truncatedList(
                        TableListComponent.getOtherNames(e),
                        n => n.designation
                    ),
                });
            }
            if (tableSetup.permissibleValues) {
                row.push({
                    ...
                    values: TableListComponent.truncatedList(
                        e.valueDomain.datatype === 'Value List'
                            ? e.valueDomain.permissibleValues
                            : undefined,
            ...
            if (tableSetup.usedBy) {
                row.push({
                    ...
                    value:
                        e.usedBy &&
                        TableListComponent.lineClip(e.usedBy.join(', ')),
                });
            }
            if (tableSetup.registrationStatus) {
                row.push({
            ...
            if (tableSetup.ids) {
                if (
                    tableSetup.identifiers &&
                    tableSetup.identifiers.length > 0
                ) {
                    ...
                    row.push({
                        css: 'ids',
                        values: TableListComponent.truncatedList<CdeId, CdeId>(
                            e.ids,
                            e2 => e2
                        ),
                    });
            ...
            if (tableSetup.updated) {
                row.push({
                    css: 'updated',
                    value: e.updated
                        ? new Date(e.updated).toLocaleString('en-US')
                        : '',
                });
            }
            if (tableSetup.tinyId) {
            ...
            if (tableSetup.linkedForms) {
                const lfSettings = this.esService.buildElasticQuerySettings(
                    new SearchSettings(e.tinyId)
                );
    ...
    static truncatedList<T, U = string>(
        list: T[] | undefined,
        f: (a: T) => U
    ): U[] {
            ...
            if (formatted) {
                result.push(
                    typeof formatted === 'string'
                        ? this.lineClip(formatted)
                        : formatted
                );
            }
            if (
                typeof formatted === 'string' &&
                result.length === this.maxLines &&
                i + 1 < size
            ) {
                result.push('...');
                i = size;
    ...
    static getQuestionTexts(e: Item): Designation[] {
        return e.designations.filter(n => {
            ...
            return (
                n.tags.filter(t => t.indexOf('Question Text') > -1).length > 0
            );
        });
    }
