...
export class FhirAppsComponent {
    ...
    deleteApp(app: FhirApp) {
        this.http.delete('/server/fhirApp/' + app._id).subscribe(() => this.loadAllApps());
    }
    ...
    loadAllApps() {
        this.http.get<FhirApp[]>('/server/fhirApps').subscribe(res => this.fhirApps = res);
    }
    ...
    saveApp(app: FhirApp) {
        this.http.post('/server/fhirApp', app).subscribe(() => this.loadAllApps());
    }
}
