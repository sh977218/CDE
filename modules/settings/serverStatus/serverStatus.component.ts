...
export class ServerStatusComponent {
    ...
    reIndex() {
        this.http
            .post('/server/system/reindex/' + this.indexToReindex, {})
            .subscribe(() => (this.isDone = true));
        const indexFn = setInterval(() => {
            this.http
                .get<any>(
                    '/server/system/indexCurrentNumDoc/' + this.indexToReindex
                )
                .subscribe(response => {
                    this.esIndices[this.indexToReindex].count = response.count;
                    this.esIndices[this.indexToReindex].totalCount =
                        response.totalCount;
                    if (
                        this.esIndices[this.indexToReindex].count >=
                            this.esIndices[this.indexToReindex].totalCount &&
                        this.isDone
                    ) {
                        ...
                        this.alert.addAlert(
                            'success',
                            'Finished reindex ' +
                                this.esIndices[this.indexToReindex].name
                        );
                        setTimeout(() => {
    ...
    refreshStatus() {
        this.http
            .get<any>('/server/siteAdmin/serverStatuses')
            .subscribe(result => {
                ...
                if (this.statuses) {
                    this.statuses.forEach(s => {
                        s.allUp =
                            s.elastic.up &&
                            s.elastic.indices.filter((ind: any) => ind.up)
    ...
    syncLinkedFormWithCdeTinyId(tinyId) {
        this.http
            .post('/server/syncLinkedFormWithTinyId', { tinyId })
            .subscribe(
                (res: any) => {
