angular.module('systemModule').controller('ApproveCommentCtrl', ['$scope', '$http', 'Mail', '$uibModal', 'AlertService', function ($scope, $http, Mail, $modal, Alert) {
    ...
    $scope.authorizeUser = function (msg) {
        ...
        $http.post('/addUserRole', request).then(function onSuccess(response) {
            Alert.addAlert("success", response.data);
        }).catch(function onError(response) {
            Alert.addAlert("danger", response.data);
        });
