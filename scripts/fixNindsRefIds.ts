        ...
        DAO.dao.find(cond).cursor({batchSize: 1000, useMongooseAggCursor: true})
            .eachAsync(elt => {
                return new Promise(async (resolveDAO, reject) => {
                    ...
                    elt.designations = dropWhile(eltObj.designations, (d: any) => {
                        return !d.designation || isEmpty(d.designation)
                    });
                    elt.designations.forEach(d => {
                        d.tags = dropWhile(d.tags, t => t === 'Health');
                    });

                    elt.definitions = dropWhile(eltObj.definitions, (d: any) => !d.definition || isEmpty(d.definition));
