...
import { getStream, update } from '../server/cde/mongo-cde';

let stream = getStream({archived: false,
...
stream.on('data', cde => {
    ...
    flatNames.forEach((fn, i) => {
        if (flatNames.indexOf(fn) !== i) {
            ...
            if (mergeTags) {
                cde.naming[dupInd].tags = uniq(cde.naming[dupInd].tags.concat(cde.naming[i].tags));
                cde.naming[i] = "";
                changed = true;
            }
        }
