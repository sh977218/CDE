...
exports.module = function () {
    ...
    router.post('/approval', [loggedInMiddleware], (req, res) => {
        ...
        boardDb.byId(boardId, handle404({req, res}, board => {
            if (!board.review.startDate) {
                return res.send('Board has not started review yet.');
            }
            if (board.review.endDate && board.review.endDate < new Date()) {
                return res.send('Board has already ended review.');
            }
            board.users.forEach(u => {
                if (u.username.toLowerCase() === req.user.username.toLowerCase()) {
    ...
    router.post('/startReview', [loggedInMiddleware], (req, res) => {
        ...
        boardDb.byId(boardId, handle404({req, res}, board => {
            ...
            board.users.forEach(u => u.status.approval = 'invited');
            board.save(handleError({req, res}, () => res.send('done')));
