...
exports.module = function (roleConfig) {
    ...
    router.put('/id/:id/forms/', nocacheMiddleware, [nocacheMiddleware, loggedInMiddleware], (req, res) => {
    ...
    router.post("/users", isAuthenticatedMiddleware, (req, res) => {
        let boardId = req.body.boardId;
    ...
    router.post("/approval", isAuthenticatedMiddleware, (req, res) => {
        let boardId = req.body.boardId;
    ...
    router.post("/startReview", isAuthenticatedMiddleware, (req, res) => {
        let boardId = req.body.boardId;
        boardDb.byId(boardId, handleError({req, res}, board => {
                if (!board) return res.status(404).send("No board found.");
    ...
    router.post("/endReview", isAuthenticatedMiddleware, (req, res) => {
        let boardId = req.body.boardId;
        boardDb.byId(boardId, handleError({req, res}, board => {
                if (!board) return res.status(404).send("No board found.");
                if (!checkBoardOwnerShip(board, req.user)) return res.status(401).send("Not Authorized.");
    ...
    router.post("/remindReview", isAuthenticatedMiddleware, (req, res) => {
        let boardId = req.body.boardId;
        boardDb.byId(boardId, handleError({req, res}, board => {
                if (!board) return res.status(404).send("No board found.");
                if (!checkBoardOwnerShip(board, req.user)) return res.status(401).send("Not Authorized.");
    ...
    router.post('/myBoards', [nocacheMiddleware, isAuthenticatedMiddleware], (req, res) => {
        elastic.myBoards(req.user, req.body, handleError({req, res}, result => res.send(result)));
    });


    router.post('/pinEntireSearchToBoard', isAuthenticatedMiddleware, (req, res) => {
        let boardId = req.body.boardId;
        boardDb.byId(boardId, handleError({req, res}, board => {
                if (!board) return res.status(404).send("No board found.");
                if (!checkBoardOwnerShip(board, req.user)) return res.status(401).send("You must own a board to do this.");
