...
export function fetchWholeForm(form: CdeForm, callback: CbError1<CdeForm>) {
    iterateFe(
        ...
        (f, cb, options) => {
            ...
            dbPlugins.form.byTinyIdAndVersion(f.inForm.form.tinyId, f.inForm.form.version).then(item => {
                /* istanbul ignore if */
                if (!item) {
    ...
    fetchWholeForm(
        ...
        splitError(callback, wholeForm => {
            ...
            iterateFe(
                wholeForm,
                (f, cb) => {
                    formModel.findOne({ tinyId: f.inForm.form.tinyId, archived: false }, { version: 1 }, null).then(
                        elt => {
...
export function batchModify(req: Request, res: Response): Promise<Response> {
    ...
    return new Promise<Response>(resolve => {
        elasticsearchForm(req.user, body.searchSettings, (err, result) => {
            ...
            consoleLog(
                `Running Batch Modify from search "/form/search?${searchSettingsElasticToQueryString(
                    body.searchSettings
                )}"` +
                    (editRegStatus
...
export const bundleCreate: FormBundleRequestHandler = (req, res) => {
    ...
    dbPlugins.form.byTinyId(tinyId).then(form => {
        ...
        dbPlugins.form
            .updatePropertiesById(form._id, { isBundle: true })
            .then(savedForm => (savedForm ? res.send(savedForm) : res.status(404).send()), respondError({ req, res }));
...
export const bundleDestroy: FormBundleRequestHandler = (req, res) => {
    ...
    dbPlugins.form.byTinyId(tinyId).then(form => {
        ...
        dbPlugins.form
            .updatePropertiesById(form._id, { isBundle: false })
            .then(savedForm => (savedForm ? res.send(savedForm) : res.status(404).send()), respondError({ req, res }));
    }, respondError({ req, res }));
};
...
export function byId(req: Request, res: Response) {
    ...
    return dbPlugins.form.byId(id).then(form => {
        ...
        fetchWholeForm(
            form,
            handleNotFound({ req, res }, wholeForm => {
                ...
                if (req.query.type === 'xml') {
                    ...
                    if (req.query.subtype === 'odm') {
                        ...
                        getFormNih(
                            wholeForm,
                            handleError({ req, res }, xmlForm => res.send(xmlForm))
                        );
                    }
                } else {
                    if (req.query.subtype === 'fhirQuestionnaire') {
                        ...
                        if (req.query.hasOwnProperty('validate')) {
                            const p = resolve(
                                global.assetDir('shared/mapping/fhir/assets/schema/Questionnaire.schema.json')
                            );
...
export function priorForms(req: Request, res: Response) {
    ...
    return dbPlugins.form.byId(id).then(form => {
        ...
        formModel
            .find({}, { 'updatedBy.username': 1, updated: 1, changeNote: 1, version: 1, elementType: 1 })
            .where('_id')
            .in(history)
            .exec()
            .then(priorForms => {
...
export function byTinyId(req: Request, res: Response) {
    ...
    return dbPlugins.form.byTinyId(tinyId).then(form => {
        ...
        fetchWholeForm(
            form,
            handleNotFound(handlerOptions, wholeForm => {
                wipeRenderDisallowed(wholeForm, req.user);
                if (req.query.type === 'xml') {
                    setResponseXmlHeader(res);
                    switch (req.query.subtype) {
                        case 'odm':
                            getFormOdm(
                                wholeForm,
                                handleError(handlerOptions, xmlForm => {
                                    res.setHeader('Content-Type', 'text/xml');
                                    return res.send(xmlForm);
                                })
                            );
                        ...
                        default:
                            getFormNih(
                                wholeForm,
                                handleError(handlerOptions, xmlForm => res.send(xmlForm))
                            );
                    }
                } else {
                    res.send(wholeForm);
...
export function byTinyIdAndVersion(req: Request, res: Response) {
    ...
    return dbPlugins.form.byTinyIdAndVersion(tinyId, version).then(form => {
        if (!form) {
            handleNotFound({ req, res })(null, null);
            return res;
        }
        ...
        .then(elt => {
            ...
            return (function getDraft(): Promise<Response> {
                return draftByTinyId(tinyId).then(draft => {
                    ...
                    return promisify(fetchWholeFormOutdated)(draft.toObject()).then(wholeForm => res.send(wholeForm));
                });
            })();
        })
...
export function forEditByTinyIdAndVersion(req: Request, res: Response) {
    ...
    dbPlugins.form.byTinyIdAndVersion(tinyId, version).then(form => {
        if (!form) {
            return res.status(404).send();
        }
        fetchWholeFormOutdated(
...
export function draftSave(req: RequestWithItem, res: Response): Response | Promise<Response> {
    ...
    return formDraftSave(elt, req.user).then(elt => {
        if (!elt) {
...
export function byTinyIdList(req: Request, res: Response): Promise<Response> {
    ...
    return dbPlugins.form
        .byTinyIdListElastic(req.params.tinyIdList.split(','))
...
export function create(req: Request, res: Response): Response | Promise<Response> {
    const elt = req.body;
    const user = req.user;
    /* istanbul ignore if */
    ...
    return formCreate(elt, user).then(dataElement => res.send(dataElement), respondError({ req, res }));
}
...
export function originalSourceByTinyIdSourceName(req: Request, res: Response): Promise<Response> {
    ...
    return formOriginalSourceByTinyIdSourceName(tinyId, sourceName).then(originalSource => {
        if (!originalSource) {
            return res.status(404).send();
        }
...
export function viewHistory(req: Request, res: Response): Promise<Response> {
    ...
    return dbPlugins.form.byTinyIdListElastic(idList).then(forms => res.send(forms), respondError({ req, res }));
}
