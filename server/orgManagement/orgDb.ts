...
export function updateOrg(org: Organization, res: Response): Promise<Response> {
    const id = org._id;
    delete org._id;
    return organizationModel.findOneAndUpdate({ _id: id }, org, { new: true }).then(
        found => (found ? res.send() : res.status(400).send('Could not update')),
        err => res.status(400).send('Could not update')
    );
}
