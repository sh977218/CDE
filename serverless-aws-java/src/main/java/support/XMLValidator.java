...
public class XMLValidator implements ErrorHandler {
  ...
  public XMLValidator(String schemaFile, String inputString) {
    try {
      Schema schema = loadSchema(schemaFile);
      Validator validator = schema.newValidator();

      validator.setErrorHandler(this);
      validator.validate(new StreamSource(new StringReader(inputString)));

      logResult(this.withErrors ? "WITH_ERRORS" : "OK");
    } catch (Exception e) {
