import {
    ...
    NotificationSettingsMedia,
    NotificationSettingsMediaType,
...
export function usersToNotify<T extends User>(
    type: NotificationSettingsType,
    medium: NotificationSettingsMediaType,
    users: T[]
): T[] {
    return users.filter(u => {
        const media = u.notificationSettings && u.notificationSettings[type];
        return (media && media[medium]) || (medium === 'drawer' && !(media && media[medium] === false)); // drawer default true, mongoose defaults missing objects
    });
}
