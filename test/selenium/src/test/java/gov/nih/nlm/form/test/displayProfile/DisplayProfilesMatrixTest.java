...
public class DisplayProfilesMatrixTest extends BaseFormTest {
    ...
    public void matrixDisplayProfile() {
        ...
        goToPreview();
        number = findElements(By.xpath("//cde-native-section-matrix//table//input[@type='radio']")).size();
        Assert.assertTrue(number > 0);
        number = findElements(By.xpath("//cde-native-section-matrix//table//input[@type='checkbox']")).size();
