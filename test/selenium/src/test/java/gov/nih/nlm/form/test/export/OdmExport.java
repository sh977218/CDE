...
public class OdmExport extends BaseFormTest {
    ...
    public void odmExport() {
        ...
        String response = get(baseUrl + "/server/form/form/Xy1kuxJqm?type=xml&subtype=odm").asString();
        ...
        response = get(baseUrl + "/server/form/formById/590cc0da5b9fd620f835b547?type=xml&subtype=odm").asString();
        for (String expectedResult : expectedResults) {
            Assert.assertTrue(response.contains(expectedResult), "missing: " + expectedResult + "\n Actual: \n " + response);
        }
    }
